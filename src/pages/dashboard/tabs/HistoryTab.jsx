...
const HistoryTab = () => {
    ...
    const handleFetchHistory = async (scanId) => {
        ...
        try {
            ...
            try {
                // 1. First, try to GET existing recommendations
                const existing = await API.get(`/scan/${scanId}/recommendations`);

                if (existing.data && existing.data.length > 0) {
                    // If they exist, just use them
                    setAssetPlans(existing.data);
                } else {
                    // 2. If the array is empty, ONLY then trigger the generation (POST)
                    const generated = await API.post(`/scan/${scanId}/recommendations`);
                    setAssetPlans(generated.data);
                }
            } catch (err) {
